...
cp "${HASH_TUI}" "${HASH_BIN}/opencode"
cp "${HASH_TUI}" "${HASH_BIN}/claude"
cp "${TMUX_WRAPPER}" "${HASH_BIN}/tmux"
cp "${HASH_TUI}" "${FAILURE_BIN}/opencode"
cp "${HASH_TUI}" "${FAILURE_BIN}/claude"
...
OSS_PANE="$(pane_for_agent "${SUCCESS_MANIFEST}" oss)"
CLAUDE_PANE="$(pane_for_agent "${SUCCESS_MANIFEST}" claude)"
if [ -z "${TMUX_SESSION}" ]; then
  echo "large-prompt smoke: manifest tmuxSession is empty" >&2
  exit 1
fi
if [ "$(printf '%s\n%s\n' "${LEFT_AGENT}" "${RIGHT_AGENT}" | sort | tr '\n' ',')" != "claude,oss," ]; then
...
wait_for_pane_text \
  ...
  "${SMOKE_SOCKET}" \
  "${OSS_PANE}" \
  "BOOTSTRAP_VERIFIED oss" >/dev/null
wait_for_pane_text \
  "${SUCCESS_TMUX_TMPDIR}" \
  "${SMOKE_SOCKET}" \
  "${CLAUDE_PANE}" \
...
bun -e '
  ...
  if (JSON.stringify(mappedAgents) !== JSON.stringify(["claude", "oss"])) {
...
if grep -E -q $'^(claude|oss)\t(BOOTSTRAP_VERIFIED|WORK_STARTED)' \
...
HASH_OSS_PANE="$(pane_for_agent "${HASH_MANIFEST}" oss)"
for _attempt in $(seq 1 40); do
  if [ -f .git/loop-smoke-oss.ready ]; then
    break
  fi
  sleep 0.25
done
...
HASH_OSS_CHARTER="$(bun -e 'const m=await Bun.file(process.argv[1]).json(); console.log(m.launchCharters.oss.path)' "${HASH_MANIFEST}")"
...
HASH_OUTPUT="$(
  wait_for_pane_text \
    ...
    "${HASH_SMOKE_SOCKET}" \
    "${HASH_OSS_PANE}" \
    "BOOTSTRAP_HASH_MISMATCH oss"
)"
if grep -Fq "WORK_STARTED oss" <<<"${HASH_OUTPUT}"; then
...
grep -Fq 'smoke fixture refused workspace creation' "${SMOKE_ROOT}/failure.err"
