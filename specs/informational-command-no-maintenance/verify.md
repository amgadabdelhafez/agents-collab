...
3. The realistic large-prompt smoke uses a clean allowlisted environment and
   proves its manifest/config state is below the disposable home only.
4. `scripts/verify.sh` passes with zero baseline failures.
