...
detects an active help/version token, call the existing parser to render the
response and return.
...
matching run appears under the captured real home.
...
3. Isolate the realistic launch smoke from ambient loop, Claude, Codex, and
   tmux state; add positive and negative storage-location assertions.
4. Run focused checks, the full repository verifier, and create a replayable
   eval with an empty baseline-failure list.
5. Commit the candidate and obtain an independent exact-SHA review. Hold live
