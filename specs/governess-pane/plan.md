# Plan: Governess Pane (LLM loop watchdog)
> Derived from spec.md. Do not contradict spec.md or constitution.md.

## Approach

Add an opt-in `--governess` mode to the existing paired tmux flow. A hidden `__governess <runId>`
...
governess tails. The LLM is never in the trigger path; it only confirms and summarizes.

## Sequence

1. **Flags + types + manifest** (`args.ts`, `types.ts`, `constants.ts`, `run-state.ts`): parse
   `--governess*` flags; add governess manifest fields. Inert until wired.
2. **Hook injection** (`hooks/settings.ts`, `hooks/emit.ts`): generators for the Claude settings
   ...
   Wire into `buildClaudeCommand` / Codex launch behind `opts.governess`.
3. **Detector** (`governess-detect.ts`, pure): given hook tails + pane hashes + clock →
   per-agent `{lastEventAge, paneStable, suspect}`. Fully unit-tested.
4. **LLM client** (`governess-llm.ts`): OpenAI-compatible POST to the configured endpoint;
   parse/validate verdict; reasoning-model token budget; timeouts → `working` fallback.
5. **Recovery** (`governess-recover.ts`): pure ladder-decision (`decideAction(state, history)`)
   + effectful executors (answer-prompt / nudge / restart) via injected tmux deps; cooldown + cap.
6. **Control loop + render** (`governess.ts`): tie 1–5 together, status board, `governess.jsonl`.
7. **Third pane** (`tmux.ts::startPairedSession`): when `opts.governess`, after the agent split,
   `split-window -v -f -l <height>` running `__governess`. No change to the non-governess path.
...
| Governess process | Hidden `__governess` subcommand in the same binary | Mirrors `codex-tmux-proxy`; one shipped artifact |
...
- **run-state** (`run-state.ts`) — manifest fields for governess pane/agent ids.
- **new: governess** (`governess*.ts`, `hooks/*`) — self-contained; imports tmux primitives only.

## Risks

- **False-positive recovery on a busy agent** → deterministic trigger + confidence gate + cooldown
  + `max-recoveries` cap + `waiting-human` exclusion + `--governess-dry-run` for validation.
- **Hook injection breaks agent startup** → emitter is best-effort, never blocks; if the settings
  file/endpoint is absent the agent launches normally (regression test: non-governess path unchanged).
...
- Semantic "is the work correct" judgment — the governess judges *liveness/progress*, not quality.
