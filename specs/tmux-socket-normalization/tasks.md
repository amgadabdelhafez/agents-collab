...
derived from it.
**Done when:**
- [x] Producer binary SHA-256, tmux version, capture command, UTC time,
      environment, and manifest SHA-256 are recorded (verify 14).
- [x] The legacy fixture is generated by removing only `tmuxSocket` from
      producer output and records source and derived SHA-256 (verify 14).
- [x] `fixture-index.json` per-file SHA-256 values match the checked-in bytes
      and the normalizer reproduces them.
- [x] No hand-authored fixture certifies the seam alone.

**Completed 2026-08-09 in D-026.** A compiled candidate produced the manifest
under a closed fake-control environment. A checked-in normalized tmux argv
trace proves the exact version/probe/new-session/cleanup sequence and binds
every session argument to the producer manifest. Clean and hostile-ambient
recaptures reproduced all three artifact hashes; compatibility tests prove the
new fixture yields the exact target and the one-field-derived legacy fixture
fails closed. Independent zero-write review passed.
