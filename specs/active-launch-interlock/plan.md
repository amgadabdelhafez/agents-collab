...
| Coordination | Guarded repository lock plus durable launch/attempt claims | Serializes scan, stale-owner recovery, allocation, and cold resume while allowing distinct workspaces after the claim is durable. |
...
- A stale lock could block launch. Mitigation: publish complete lock records
  atomically and serialize owner validation, reclamation, and replacement under
  a fail-closed recovery guard.
