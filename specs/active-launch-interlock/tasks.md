...
- [x] **T-03 Single-flight gate** — Reject live, unknown, legacy, same-root, or same-branch conflicts before any task/agent side effect while preserving explicit resume.
- [x] **T-04 Owned cleanup** — Ensure a failed launcher only kills a tmux session it positively created.
- [x] **T-05 Regression proof** — Add focused, barrier-concurrency, CLI ordering, manifest round-trip, and compiled-smoke coverage.
