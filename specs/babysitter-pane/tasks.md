# Tasks: Babysitter Pane (LLM loop watchdog)
> Each task must be independently implementable in one worktree.
> Tasks must not reference chat context.

## Checklist

- [ ] **T-01** Flags, types, manifest — parse `--babysit*`, add defaults + manifest fields (inert).
...
- [ ] **T-06** Control loop + render + `babysitter.jsonl` — the `__babysit` subcommand body.
- [ ] **T-07** Third pane wiring — full-width bottom pane in `startPairedSession` behind `opts.babysit`.
...
**Goal:** CLI accepts `--babysit`, `--babysit-idle`, `--babysit-cooldown`, `--babysit-max-recoveries`,
`--babysit-dry-run`, `--babysit-url`, `--babysit-model`, `--babysit-height`; `Options` + manifest carry them.
...
- [ ] Manifest gains `tmuxPaneBabysitAgent` + babysitter pane id fields.
- [ ] Tests pass for touched code.

### T-02 — Hook injection
**Goal:** At launch (when `opts.babysit`), Claude gets `--settings <runDir>/hooks/claude-settings.json`
...
**Done when:**
- [ ] Non-babysit launch is unchanged (no `--settings`, no config.toml written).
...
**Files:** `src/loop/babysitter-detect.ts`, `tests/loop/babysitter-detect.test.ts`
...
**Files:** `src/loop/babysitter-llm.ts`, `tests/loop/babysitter-llm.test.ts`
...
**Files:** `src/loop/babysitter-recover.ts`, `tests/loop/babysitter-recover.test.ts`
**Inputs:** verdict, per-agent action history, cooldown/cap/confidence from options.
**Output:** action taken (or none) + updated history; each execution appends to `babysitter.jsonl`.
**Done when:**
...
**Goal:** The `__babysit <runId>` subcommand: tick every interval → detect → judge suspects → recover →
render status board → append decisions to `runs/<id>/babysitter.jsonl`.
**Files:** `src/loop/babysitter.ts`, `src/loop/cli.ts` (register `__babysit`), `tests/loop/babysitter.test.ts`
**Inputs:** T-03..T-05 modules; run-state; tmux primitives.
**Output:** live pane board + `babysitter.jsonl`.
**Done when:**
...
**Goal:** When `opts.babysit`, add a full-width bottom pane running `__babysit` after the agent split.
**Files:** `src/loop/tmux.ts`, `tests/loop/tmux.test.ts`
**Inputs:** `startPairedSession`; `split-window -v -f -l <height>`.
**Output:** 3-pane session; manifest records the babysitter pane.
**Done when:**
- [ ] Without `--babysit`, `startPairedSession` behavior is byte-for-byte unchanged (asserted).
- [ ] With `--babysit`, bottom pane spans full width under both agents.
