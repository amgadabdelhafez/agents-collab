...
3. Register current Codex/Claude tool and subagent hooks. Gate native spawn
   calls, enforce the one provider-enforceable read-only profile, and deny
   unsafe child tools and descendants.
4. Generate loop-scoped provider controls: Codex native agents disabled in
   governed modes because 0.145 inherits the parent sandbox, plus Claude's
   read-only CLI profile or strict deny flags.
